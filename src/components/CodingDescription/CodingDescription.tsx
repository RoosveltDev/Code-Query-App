...
import { CodingDescriptionType } from '../../types/liveCoding/liveCodingDescription'
import { fetchedQuestions } from '../../types/question/fetchedQuestions.type'
import QuestionContent from '../QuestionContent/QuestionContent'
import './codingDescription.css'
import AnswerCard from '../AnswerCard/AnswerCard'

const CodingDescription = ({answerId,questionId}:CodingDescriptionType) => {
  const [isQuestion,setIsQuestion] = useState<boolean>(true)
  const [answerData] = useFetch<Answer>({fetchOptions:{
    context: `answers/${answerId}`,
    method: "GET",
    data: {},
    hasCredentials: true,
    bodyFormat: "row" ,
  }})
  const [questionData] = useFetch<fetchedQuestions>({fetchOptions:{
  ...
  return (
    <div className='coding-description-main-container'>
        <div className='coding-description-main-container__header header-coding-description-container'>
            <div onClick={()=> setIsQuestion(true)} className='header-coding-description-container__element element-header-coding-container element-header-coding-container--first'>
            ...
            <div onClick={()=> setIsQuestion(false)} className='header-coding-description-container__element element-header-coding-container'>
        ...
        </div>
        {
          isQuestion
          ? (
            <div className='coding-description-main-container__section coding-description-main-container__section--question'>
             {questionData && <QuestionContent question={questionData!}></QuestionContent>}
           </div>
          )
          :(
            <div className='coding-description-main-container__section coding-description-main-container__section--question'>
             {answerData && <AnswerCard created_at={answerData.created_at} is_accepted={answerData.is_accepted} body={answerData.body} id={answerData.id} classroom_id={answerData.classroom_id} user={answerData.user}></AnswerCard>}
           </div> 
          )
        }
