...
const Dashboard = () => {
  ...
  const [dataChart, setDataChart] = useState<ChartDatasetType>({
    labels: [],
    datasets: [],
  });
  const [dataChartInteractions, setDataChartInteractions] =
    useState<ChartDatasetType>({ labels: [], datasets: [] });
    ...
    <div className="dashboard-main-container">
      ...
      <div className="dashboard-main-container__header dashboard-header-container">
        ...
        <select
          ...
          onChange={(e) => handleSelectDashboard(e, setIndexSelected)}
          name="Classrooms"
        ...
        >
          {data &&
            data.map((element, index) => {
              return (
                <option key={`option${element.id}`} value={index}>
      ...
      <div className="dashboard-main-container__courses dashboard-courses-container">
        <div className="dashboard-courses-container__header">
          ...
          <button
            onClick={() =>
              handleClickMore(controllerRef, setData, data!.length)
            }
            className="dashboard-header-container__button"
          >
            See more <FaChevronCircleRight size={14}/>
          </button>
        </div>
        <div className="dashboard-courses-container__list courses-list-dashboard">
          ...
          <div className="courses-list-dashboard__courses">
            {data &&
              data.map((element, index) => {
                return (
                  <CardList<FetchedClassroomsType>
                    key={element.id}
                    cardData={element}
                    index={index}
                    elementCard={[
                      index === 0 || index === 1 || index === 2 ? (
                        <img
                          className="classroom-card-container__image"
                          src={position}
                          alt="Top Logo"
                        ></img>
                      ) : (
